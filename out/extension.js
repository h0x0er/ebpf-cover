...
function doCover(context) {
    ...
    let lastLine = {
        ...
        rangeIncludingLineBreak: new vscode.Range(new vscode.Position(0, 0), new vscode.Position(0, 0)),
    ...
    if (currentRanges !== undefined) {
        ...
        const verifierLogFile = path.join(workspace, utils_1.VerifierFileName);
        if (!fs.existsSync(verifierLogFile)) {
            (0, utils_1.Log)(`[doCover] verifier log-file not found path=${verifierLogFile}`);
        }
